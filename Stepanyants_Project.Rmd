...
## Введение
В древнеармянском языке есть единица ayl, тогда как в древнегреческом есть когнатные ей alla/allos (диахронически связанные: союз возник из "местоимения"). Известно, что др.-арм. переводы Евангелий делались главным образом с др.-греч. языка (в первую очередь в 5 веке) и что многие конструкции были калькированы. Др.-арм. единица ayl достаточно частотна (служебное слово), имеет, как и др.-греч. alla, три функции: 1) уступительная 'though', 2) заместительная 'but rather', 3) неидентичность 'other, else' (согласно моим наблюдениям по корпусу и изучению словарей; иногда выделяются другие, но, как кажется, они более периферийны).

Исследование опирается на дипломную работу автора. В имеющейся индоевропеистической литературе по теме статистические выводы не делались, см. Klein, Jared S. 2022. Adversative Conjunction and Neighboring Discourse Features in Old Church Slavic (Codex Marianus), with Comparative Notes on the Same Phenomena in Greek, Gothic and Classical Armenian. Transactions of the Philological Society 120(1). 128–183. (doi:10.1111/1467-968X.12231).
...
## Методы анализа
Статистический тест Хи-квадрат Пирсона. Применяется для оценки связи между совместно встречающимися переменными. Необходимые допущения: случайная выборка данных, независимость наблюдений, не слишком мало наблюдений. (Ещё можно посчитать Точный тест Фишера, т.к. табличка сопряжённости маленькая.) Применяется к таблицам сопряжённости для каждой из трёх функций и наличия в др.-греч. тексте единицы alla/ allos. Рассматриваем отдельно все данные вместе и два Евангелия.
...
Неправильно применять тест, т.к. в одной ячейке всего 4 наблюдения
...
Неправильно применять тест, т.к. есть 3 наблюденпия.
...
Гипотезы подтвердились лишь частично. Для дальнейшего развития исследования (насколько это возможно) полезно было бы увеличить объёмы выборок (самое понятное - взять ещё два доступных  Евангелия), учесть данные о переводчиках, проверить разметку. Для более правильной лингвистической интерпретации нужно привлекать данные из непереводных текстов. Можно использовать Точный тест Фишера. Базовые сравнения вроде частотности осложнены необходимостью разметки на двух языках (важны конкретные функции).